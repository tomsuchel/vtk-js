...
function vtkSplineTubeContextRepresentation(publicAPI, model) {
  // Set our className
  ...
  model.pipelines.border.tubeFilter.setCapping(false);
  model.pipelines.border.tubeFilter.setNumberOfSides(50);
  model.pipelines.border.tubeFilter.setRadius(0.05);
  ...
  model.pipelines.border.tubeFilter.setInputConnection(
    publicAPI.getOutputPort()
  );
  model.pipelines.border.cutter.setInputConnection(
    model.pipelines.border.tubeFilter.getOutputPort()
  );
  model.pipelines.border.mapper.setInputConnection(
    model.pipelines.border.cutter.getOutputPort()
  );
  ...
  publicAPI.requestData = (inData, outData) => {
    ...
    model.pipelines.border.cutter.setCutFunction(model.plane);
  };
  ...
  // publicAPI.setPlane = (plane) => {
  //   model.plane = plane;
  //   model.pipelines.border.cutter.setCutFunction(plane);
  // };
}
...
export function extend(publicAPI, model, initialValues = {}) {
  ...
  vtkSplineTubeContextRepresentation(publicAPI, model);
}
...
export const newInstance = macro.newInstance(
  extend,
  'vtkSplineTubeContextRepresentation'
);
